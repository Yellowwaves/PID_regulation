...
void visualisationC(float puissance) // Cette fonction permet de récupérer la consigne, elle est affectée par le verrou et elle write true/false dans le fichier data
{
   if (access(".verrouData", F_OK)) 
   {
    // .verrouData n'existe pas, donc on peut écrire dans data
      ...
      FILE *fdata;
      fdata = fopen ("data.txt", "r"); // on ouvre le fichier data en mode lecture


      char lines[3][10];// on crée un tableau de 2 lignes de 10 caractères max, pour stocker temporairement les lignes du fichier data.txt

      int numero_ligne = 0;
      while (fgets(lines[numero_ligne], 10, fdata) != NULL) {//la boucle while permet de stocker les lignes du fichier data.txt dans le tableau lines
         numero_ligne++;
      }
      fclose (fdata);
      ...
      fprintf(fdata, "%s", lines[1]); //on réécrit les lignes de températures (pour le pas écraser les anciennes données) dans le fichier data.txt
